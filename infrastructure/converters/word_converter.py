...
class WordConverter(Converter):
    """
    Converter for converting PDFs to Word documents.
    """
    ...
    def convert(self, document: PDFDocument, options: ConversionOptions) -> Path:
        """
        Convert a PDF document to Word format.
        ...
        if not self.validate_conversion(options):
            raise ValueError("Invalid conversion options for Word conversion")
        
        try:
            # Ensure output directory exists
            options.output_path.parent.mkdir(parents=True, exist_ok=True)
            
            # Convert PDF to Word using pypandoc
            ...
            pypandoc.convert_file(
                ...
                extra_args=['--extract-media', str(output_path.parent / 'media')]
            )
            
            self.logger.info(f"Successfully converted PDF to Word: {output_path}")
            return output_path
            
        except Exception as e:
            self.logger.error(f"Failed to convert PDF to Word: {str(e)}")
            raise ValueError(f"Word conversion failed: {str(e)}")
    ...
    def validate_conversion(self, options: ConversionOptions) -> bool:
        ...
        try:
            ...
            if options.type != ConversionType.WORD:
                return False
